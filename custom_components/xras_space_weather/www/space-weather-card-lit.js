...
class SpaceWeatherCardLit extends LitElement {
  ...
  static get styles() {
    return css`
      ...
      .tile-row { display: flex; justify-content: space-between; align-items: center; padding: 12px 16px; background: var(--secondary-background-color); border-radius: var(--ha-card-border-radius, 12px); }
      .tile-col { display: flex; flex-direction: column; gap: 4px; padding: 12px 16px; background: var(--secondary-background-color); border-radius: var(--ha-card-border-radius, 12px); }
  ...
  render() {
    ...
    return html`
      <div class="card-config">
        ...
        <ha-textfield label="${this.t.editor.city_label}" .value=${this._config.city || ""} .configValue=${"city"} @input=${this._valueChanged}></ha-textfield>
      </div>
    `;
  }
  _valueChanged(ev) {
...
customElements.define("space-weather-card-lit-editor", SpaceWeatherCardLitEditor);

window.customCards = window.customCards || [];
if (!window.customCards.some(c => c.type === 'space-weather-card-lit')) {
  const lang = (navigator.language || 'en').substring(0, 2);
  const t = TRANSLATIONS[lang] || TRANSLATIONS['en'];
  window.customCards.push({ type: "space-weather-card-lit", name: t.card_name, description: t.card_desc, preview: true });
}
