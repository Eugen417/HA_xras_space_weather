console.info(
  "%c 🌌 SPACE-WEATHER-CARD %c v3.0.0 (Ultimate) ",
...
const TRANSLATIONS = {
  en: {
    ...
    card_desc: "Ultimate card for IKI RAN space weather (Tabs & Classic)",
    editor: {
      title: "Space Weather Card Setup",
      city_label: "City Name Override (Optional)",
      layout_label: "Display Style",
      layout_tabs: "Modern (Tabs)",
      layout_classic: "Classic (Single Page)"
    }
  },
  ru: {
    ...
    card_desc: "Универсальная карточка ИКИ РАН (Вкладки и Классика)",
    editor: {
      title: "Настройка карточки",
      city_label: "Название города (Необязательно)",
      layout_label: "Стиль отображения",
      layout_tabs: "Современный (Вкладки)",
      layout_classic: "Классика (Одной страницей)"
    }
  }
};
...
class SpaceWeatherCard extends LitElement {
  ...
  setConfig(config) {
    ...
    this.config = { layout: "tabs", ...config }; // По умолчанию вкладки
  }
  ...
  render() {
    ...
    // ЕСЛИ ВЫБРАНЫ ВКЛАДКИ (ПО УМОЛЧАНИЮ)
    return html`
      <ha-card>
        <div class="tabs-container">
            <div class="tab ${this._activeTab === 'summary' ? 'active' : ''}" @click=${() => this._switchTab('summary')}><ha-icon icon="mdi:earth"></ha-icon> ${this.t.tabs.summary}</div>
            <div class="tab ${this._activeTab === 'details' ? 'active' : ''}" @click=${() => this._switchTab('details')}><ha-icon icon="mdi:chart-bar"></ha-icon> ${this.t.tabs.details}</div>
            <div class="tab ${this._activeTab === 'forecast' ? 'active' : ''}" @click=${() => this._switchTab('forecast')}><ha-icon icon="mdi:calendar-clock"></ha-icon> ${this.t.tabs.forecast}</div>
        </div>
        <div class="tab-content">
            ${this._activeTab === 'summary' ? renderHeader() : ''}
            ${this._activeTab === 'details' ? html`<div class="content-body">${renderDetails()}</div>` : ''}
            ${this._activeTab === 'forecast' ? html`<div class="content-body">${renderForecast()}</div>` : ''}
        </div>
      </ha-card>
    `;
  }
  ...
  static get styles() {
    return css`
      ...
      .tab { flex: 1; text-align: center; padding: 12px 0; cursor: pointer; font-size: 13px; font-weight: 500; color: var(--secondary-text-color); transition: background-color 0.2s; display: flex; align-items: center; justify-content: center; gap: 6px; }
      ...
      .header-overlay { width: 100%; position: relative; z-index: 1; background: linear-gradient(to top, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.2) 70%, transparent 100%); padding: 16px; color: white; }
      ...
      .kp-desc { font-size: 16px; font-weight: 500; margin-top: 6px; text-shadow: 1px 1px 2px rgba(0,0,0,0.8); display: flex; align-items: center; gap: 8px; }
      .status-badge { width: 10px; height: 10px; border-radius: 50%; display: inline-block; box-shadow: 0 0 4px rgba(0,0,0,0.5); }
      .content-body { padding: 16px; display: flex; flex-direction: column; gap: 8px; }
...
// -------------------------------------------------------------
// 3. ВИЗУАЛЬНЫЙ РЕДАКТОР С ПЕРЕКЛЮЧАТЕЛЕМ ДИЗАЙНА
// -------------------------------------------------------------
const mwcSelectStyle = `
  ...
  render() {
    ...
    return html`
      <div class="card-config">
        ...
        <ha-select
          label="${this.t.editor.layout_label}"
          .value=${this._config.layout || "tabs"}
          .configValue=${"layout"}
          @selected=${this._valueChanged}
          @closed=${(e) => e.stopPropagation()}
        >
          <mwc-list-item value="tabs">${this.t.editor.layout_tabs}</mwc-list-item>
          <mwc-list-item value="classic">${this.t.editor.layout_classic}</mwc-list-item>
        </ha-select>
